...
def gen_pdf(titulo, content, imagens=None, tema="padrao", uma_pagina=False, sem_circulos=False, subtitulo="", tamanho_fonte="normal", espacamento="normal", hero_altura="normal", mostrar_rodape=True):
    ...
    _top_margin   = _hero_band + 0.8*cm   # margem superior = hero + folga mínima para conteúdo
    # Altura estimada do bloco de texto no hero (título + subtítulo + data + spacer)
    _hero_text_h  = 1.8*cm if not uma_pagina else 1.5*cm
    # Spacer negativo: sobe da borda superior do frame até o centro vertical do hero menos metade do texto
    sp_hero       = _hero_band - (_hero_band - _hero_text_h) / 2.0
    sp_after_hero = (_hero_band - _hero_text_h) / 2.0 + 0.6*cm if not uma_pagina else 0.4*cm
