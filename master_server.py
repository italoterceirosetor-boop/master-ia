...
def chat_tools():
    ...
    # ── Pré-processa arquivos: extrai texto ANTES de limpar o histórico ────────
    def _preprocessar_arquivo(m):
        role = m.get("role","")
        cont = m.get("content","")
        if role != "user":
            return m
        if isinstance(cont, list):
            ...
            for b in cont:
                if b.get("type") == "text":
                    t2 = _extrair_arquivo_de_texto(b.get("text",""))
                    new_parts.append({"type":"text","text":t2})
                else:
                    new_parts.append(b)
            return {"role": role, "content": new_parts}
    ...
    def _extrair_arquivo_de_texto(txt):
        ...
        return pat.sub(_substituir, txt)

    messages_proc = [_preprocessar_arquivo(m) for m in messages]
    msgs_loop = [m for m in (_limpar_msg(m) for m in messages_proc) if m]
