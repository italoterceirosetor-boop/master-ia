...
def gen_pdf(titulo, content, imagens=None, tema="padrao", uma_pagina=False, sem_circulos=False,
            subtitulo="", tamanho_fonte="normal", espacamento="normal", hero_altura="normal", mostrar_rodape=True):
    """Gera PDF profissional Master IA — design rico e adaptativo."""
    buf = io.BytesIO()

    # ── Paletas de temas ──────────────────────────────────────────────
    _temas = {
        "padrao": {"bg":"#0f172a","navy":"#1e3a5f","blue":"#2563eb","cyan":"#06b6d4","gold":"#f59e0b","accent":"#06b6d4"},
        "verde":  {"bg":"#064e3b","navy":"#022c22","blue":"#059669","cyan":"#10b981","gold":"#f59e0b","accent":"#10b981"},
        "roxo":   {"bg":"#1e1b4b","navy":"#312e81","blue":"#4f46e5","cyan":"#8b5cf6","gold":"#f59e0b","accent":"#8b5cf6"},
        "escuro": {"bg":"#111827","navy":"#1f2937","blue":"#374151","cyan":"#e5e7eb","gold":"#9ca3af","accent":"#e5e7eb"},
    }
    t = _temas.get(tema, _temas["padrao"])

    # Cores como objetos
    C = {k: colors.HexColor(v) for k, v in t.items()}
    C.update({
        'white':  colors.white,
        'text':   colors.HexColor('#1e293b'),
        'text2':  colors.HexColor('#64748b'),
    ...
    # ── Escalas ───────────────────────────────────────────────────────
    _fs  = {"pequeno":0.78,"normal":1.0,"grande":1.22,"muito_grande":1.45}.get(tamanho_fonte,1.0)
    _sp  = {"compacto":0.7,"normal":1.0,"espacoso":1.35}.get(espacamento,1.0)
    _hcm = {"pequeno":3.2,"normal":5.8,"grande":8.0}.get(hero_altura,5.8)
    sc   = (0.82 if uma_pagina else 1.0) * _fs

    def _ps(name, **kw): return ParagraphStyle(name, **kw)

    # ── Estilos adaptativos ───────────────────────────────────────────
    S = {
        'hero':   _ps('h', fontSize=round(28*_fs),  fontName='Helvetica-Bold', textColor=C['white'], alignment=TA_CENTER, leading=round(36*_fs)),
        'subtag': _ps('st',fontSize=round(9*_fs),   fontName='Helvetica',      textColor=colors.HexColor('#94a3b8'), alignment=TA_CENTER, leading=12),
        'h1':     _ps('h1',fontSize=round(11*sc,1), fontName='Helvetica-Bold', textColor=C['navy'],  leading=round(15*sc,1), spaceBefore=round(10*sc,1), spaceAfter=round(4*sc,1)),
        'h2':     _ps('h2',fontSize=round(9.5*sc,1),fontName='Helvetica-Bold', textColor=C['blue'],  leading=round(13*sc,1), spaceBefore=round(8*sc,1),  spaceAfter=round(3*sc,1)),
        'h3':     _ps('h3',fontSize=round(9*sc,1),  fontName='Helvetica-Bold', textColor=C['text'],  leading=round(13*sc,1), spaceBefore=round(5*sc,1),  spaceAfter=round(2*sc,1)),
        'body':   _ps('bd',fontSize=round(9*sc,1),  fontName='Helvetica',      textColor=C['text'],  leading=round(13.5*sc,1), alignment=TA_JUSTIFY, spaceAfter=round(3*sc,1)),
        'bullet': _ps('bu',fontSize=round(9*sc,1),  fontName='Helvetica',      textColor=C['text'],  leading=round(13*sc,1), leftIndent=12, spaceAfter=round(2*sc,1)),
        'num':    _ps('nu',fontSize=round(9*sc,1),  fontName='Helvetica',      textColor=C['text'],  leading=round(13*sc,1), leftIndent=14, firstLineIndent=-10, spaceAfter=round(2*sc,1)),
        'code':   _ps('co',fontSize=round(8.5*sc,1),fontName='Courier',        textColor=C['cyan'],  leading=round(13*sc,1), alignment=TA_LEFT),
        'cite':   _ps('ci',fontSize=round(8.5*sc,1),fontName='Helvetica-Oblique',textColor=C['blue'],leading=round(12*sc,1), leftIndent=10),
        'th':     _ps('th',fontSize=8.5,            fontName='Helvetica-Bold', textColor=C['white'], alignment=TA_CENTER, leading=11),
        'td':     _ps('td',fontSize=8.5,            fontName='Helvetica',      textColor=C['text'],  alignment=TA_LEFT,   leading=11),
        'tdc':    _ps('tc',fontSize=8.5,            fontName='Helvetica',      textColor=C['text'],  alignment=TA_CENTER, leading=11),
        'tdr':    _ps('tr',fontSize=8.5,            fontName='Helvetica',      textColor=C['text'],  alignment=TA_RIGHT,  leading=11),
        'foot':   _ps('fo',fontSize=7,              fontName='Helvetica',      textColor=colors.HexColor('#94a3b8'), alignment=TA_CENTER, leading=10),
        'aviso':  _ps('av',fontSize=round(8.5*sc,1),fontName='Helvetica',      textColor=C['amber'], leading=round(12*sc,1)),
    }

    # ── Canvas com fundo ──────────────────────────────────────────────
    class MasterDoc(BaseDocTemplate):
        ...
        def _bg(self, c, doc):
            ...
            hb = _hcm * cm
            c.saveState()
            c.setFillColor(C['bg']);   c.rect(0, H2-hb, W2, hb, fill=1, stroke=0)
            c.setFillColor(C['cyan']); c.rect(0, H2-0.28*cm, W2, 0.28*cm, fill=1, stroke=0)
            c.setFillColor(C['gold']); c.rect(0, H2-hb-0.14*cm, W2, 0.14*cm, fill=1, stroke=0)
            if not sem_circulos:
                cy = H2 - hb/2
                c.setFillColor(colors.HexColor(t['navy'])); c.circle(W2-0.7*cm, cy, 2.4*cm, fill=1, stroke=0)
                c.setFillColor(C['blue']);  c.circle(W2-0.7*cm, cy, 1.5*cm, fill=1, stroke=0)
                c.setFillColor(C['cyan']);  c.circle(W2-0.7*cm, cy, 0.55*cm,fill=1, stroke=0)
            c.setFillColor(C['cyan']);  c.rect(0, 0, 0.16*cm, H2-hb-0.14*cm, fill=1, stroke=0)
            c.setFillColor(C['bg']);    c.rect(0, 0, W2, 1.15*cm, fill=1, stroke=0)
            c.setFillColor(C['cyan']);  c.rect(0, 1.15*cm, W2, 0.09*cm, fill=1, stroke=0)
            c.restoreState()

    hb_cm_val = _hcm * cm
    top_margin = hb_cm_val + 1.0*cm
    doc = MasterDoc(buf, pagesize=A4,
        leftMargin=1.8*cm, rightMargin=1.8*cm,
        topMargin=top_margin, bottomMargin=1.8*cm,
    ...
    def _md2rl(text):
        ...
        text = _re.sub(r'\*\*(.+?)\*\*', r'<b>\1</b>', text)
        text = _re.sub(r'\*(.+?)\*',      r'<i>\1</i>', text)
        text = _re.sub(r'`([^`]+)`',       r'<font name="Courier" size="8.5" color="'+t['cyan']+'">\\1</font>', text)
        text = text.replace('`', '')
        text = _re.sub(r'\[([^\]]+)\]\([^)]+\)', r'\1', text)
        return text
        ...
        while i < len(lines):
            ...
            if stripped.startswith('```'):
                ...
                while i<len(lines) and not lines[i].strip().startswith('```'):
                    code_lines.append(lines[i]); i+=1
                if i<len(lines): i+=1
                if code_lines: blocks.append({'type':'codeblock','text':' '.join(l.strip() for l in code_lines if l.strip())})
                continue
            if '|' in stripped and i+1<len(lines) and _re.match(r'^\|[-| :]+\|', lines[i+1].strip()):
                headers=[c.strip() for c in stripped.split('|') if c.strip()]; i+=2; rows=[]
                while i<len(lines) and '|' in lines[i]:
                    r=[c.strip() for c in lines[i].split('|') if c.strip()]
                    if r: rows.append(r)
                    i+=1
                blocks.append({'type':'table','headers':headers,'rows':rows}); continue
            m=_re.match(r'^(#{1,4})\s+(.+)',stripped)
            if m: blocks.append({'type':f'h{len(m.group(1))}','text':_md2rl(m.group(2).strip())}); i+=1; continue
            if _re.match(r'^---+$',stripped): blocks.append({'type':'hr'}); i+=1; continue
            if stripped.startswith('>'):
                blocks.append({'type':'cite','text':_md2rl(stripped.lstrip('>').strip())}); i+=1; continue
            if _re.match(r'^[-*•]\s+',stripped):
                items=[]
                while i<len(lines) and _re.match(r'^[-*•]\s+',lines[i].strip()):
                    items.append(_md2rl(lines[i].strip()[2:].strip())); i+=1
            ...
            while i<len(lines):
                ...
                if (not l or l.startswith('#') or l.startswith('>') or
                    _re.match(r'^[-*•]\s+',l) or _re.match(r'^\d+\.\s+',l) or
                    _re.match(r'^---+$',l) or l.startswith('```') or
                    ('|' in l and i+1<len(lines) and _re.match(r'^\|[-| :]+\|',lines[i+1].strip() if i+1<len(lines) else ''))): break
                para.append(l); i+=1
            txt=' '.join(para).strip()
            if txt: blocks.append({'type':'para','text':_md2rl(txt)})
        return blocks
    ...
    # ── Hero ──────────────────────────────────────────────────────────
    hero_h = _hcm * cm
    story.append(Spacer(1, -(hero_h - 2.2*cm)))
    story.append(Paragraph(_md2rl(titulo or 'Documento'), S['hero']))
    _sub = subtitulo
    if not _sub:
        for b in blocks:
            if b['type']=='para' and len(_re.sub(r'<[^>]+>','',b.get('text',''))) < 100:
                _sub = _re.sub(r'<[^>]+>','',b['text'])[:80]; break
    if _sub: story.append(Paragraph(_sub, S['subtag']))
    story.append(Paragraph(f'Gerado por Master IA · {now_str()}', S['subtag']))
    story.append(Spacer(1, hero_h + 0.6*cm))

    # ── Conteúdo ──────────────────────────────────────────────────────
    sp_sec = round(0.18*_sp,2)*cm if not uma_pagina else 0.08*cm

    for block in blocks:
        btype=block.get('type'); txt=block.get('text','')

        if btype=='h1':
            t2=Table([[Paragraph(txt,S['h1'])]],colWidths=[CW])
            t2.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),colors.HexColor('#eff6ff')),
                ('LINEBEFORE',(0,0),(0,-1),4,C['blue']),
                ('TOPPADDING',(0,0),(-1,-1),7),('BOTTOMPADDING',(0,0),(-1,-1),7),('LEFTPADDING',(0,0),(-1,-1),10)]))
            story.append(Spacer(1,4)); story.append(t2); story.append(Spacer(1,4))

        elif btype=='h2':
            story.append(Paragraph(txt,S['h2']))
            story.append(HRFlowable(width='100%',thickness=0.7,color=C['surf2'],spaceAfter=3))

        elif btype=='h3': story.append(Paragraph(txt,S['h3']))

        elif btype=='para': story.append(Paragraph(txt,S['body']))

        elif btype=='bullets':
            for item in block['items']:
                story.append(Paragraph(f'<font color="{t["blue"]}" size="11">•</font>  {item}',S['bullet']))

        elif btype=='numbered':
            for n,item in enumerate(block['items'],1):
                story.append(Paragraph(f'<b><font color="{t["blue"]}">{n}.</font></b>  {item}',S['num']))

        elif btype=='cite':
            t2=Table([[Paragraph(txt,S['cite'])]],colWidths=[CW])
            t2.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),C['surf']),
                ('LINEBEFORE',(0,0),(0,-1),3,C['cyan']),
                ('TOPPADDING',(0,0),(-1,-1),5),('BOTTOMPADDING',(0,0),(-1,-1),5),('LEFTPADDING',(0,0),(-1,-1),10)]))
            story.append(t2); story.append(Spacer(1,3))

        elif btype=='codeblock':
            s_code=_ps(f'mc{tema}',fontSize=round(8.5*sc,1),fontName='Courier',textColor=C['cyan'],leading=round(13*sc,1),alignment=TA_LEFT)
            t2=Table([[Paragraph(txt,s_code)]],colWidths=[CW])
            t2.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),colors.HexColor('#0d1b2a')),
                ('LINEBEFORE',(0,0),(0,-1),3,C['cyan']),
                ('TOPPADDING',(0,0),(-1,-1),8),('BOTTOMPADDING',(0,0),(-1,-1),8),
                ('LEFTPADDING',(0,0),(-1,-1),12),('RIGHTPADDING',(0,0),(-1,-1),8)]))
            story.append(Spacer(1,3)); story.append(t2); story.append(Spacer(1,3))

        elif btype=='table':
            headers=block.get('headers',[]); rows=block.get('rows',[])
            if not headers: continue
            ncols=len(headers)
            lens=[max(len(str(headers[j])),max((len(str(r[j])) if j<len(r) else 0) for r in rows) if rows else 3) for j in range(ncols)]
            total=sum(lens) or 1
            cws=[max(CW*l/total,CW*0.08) for l in lens]
            sw=sum(cws); cws=[w*CW/sw for w in cws]
            fs=7.5 if ncols>5 else 8.5
            uid=str(abs(hash(str(headers))))[-4:]
            ...
            s_tr2=_ps(f'TR{uid}',fontSize=fs,fontName='Helvetica',textColor=C['text'],alignment=TA_RIGHT,leading=fs+4)
            def _is_num(v):
            ...
            tdata=[[Paragraph(str(h),s_th2) for h in headers]]
            for r in rows:
                tdata.append([Paragraph(str(r[j]) if j<len(r) else '',s_tr2 if j in num_cols else s_td2) for j in range(ncols)])
            t2=Table(tdata,colWidths=cws,repeatRows=1,hAlign='CENTER',splitByRow=True)
            ...
            story.append(Spacer(1,sp_sec))

    if mostrar_rodape:
        story.append(Spacer(1,0.5*cm))
        story.append(HRFlowable(width='100%',thickness=0.4,color=C['surf2'],spaceAfter=3))
        story.append(Paragraph('Gerado por Master IA · Este documento tem caráter informativo.',S['foot']))
...
def gen_word(titulo, content):
    ...
    # Cabeçalho Master — banner azul com título
    if titulo:
        # Tabela banner
    ...
    for block in blocks:
        ...
        if btype == 'h1':
            ...
            _word_add_border_bottom(p, '2563eb', 6)
            ...
            for part in parts:
                if part.startswith('**') and part.endswith('**'):
                    r = p.add_run(part[2:-2]); r.bold = True; r.font.size = Pt(10.5); r.font.name = 'Calibri'
                elif part.startswith('*') and part.endswith('*'):
                    r = p.add_run(part[1:-1]); r.italic = True; r.font.size = Pt(10.5); r.font.name = 'Calibri'
                elif part.startswith('`') and part.endswith('`'):
                    r = p.add_run(part[1:-1])
                    r.font.name = 'Courier New'; r.font.size = Pt(9.5)
                    r.font.color.rgb = RGBColor(0x25,0x63,0xeb)
                elif part:
                    r = p.add_run(part); r.font.size = Pt(10.5); r.font.name = 'Calibri'
        elif btype == 'bullets':
            for item in block['items']:
                p = doc.add_paragraph(style='List Bullet')
                p.paragraph_format.space_after = Pt(2)
                item = _limpar_tags_rl(item)
                ...
                for part in parts:
                    if part.startswith('**') and part.endswith('**'):
                        ...
                        r = p.add_run(part)
                    if part: r.font.size = Pt(10.5); r.font.name = 'Calibri'
        elif btype == 'numbered':
            for item in block['items']:
            ...
            raw_cite = block.get('text','')
            tbl_cite = doc.add_table(rows=1, cols=1)
            tbl_cite.style = 'Table Grid'
            cc = tbl_cite.rows[0].cells[0]
            _word_set_cell_bg(cc, 'f1f5f9')
            tc = cc._tc; tcPr = tc.get_or_add_tcPr()
            tcBorders = OxmlElement('w:tcBorders')
            left_b = OxmlElement('w:left')
            left_b.set(qn('w:val'), 'single')
            left_b.set(qn('w:sz'), '18')
            left_b.set(qn('w:color'), '06b6d4')
            tcBorders.append(left_b); tcPr.append(tcBorders)
            rc = cc.paragraphs[0].add_run(_re.sub(r'<[^>]+>', '', raw_cite))
            rc.font.size = Pt(9.5); rc.font.italic = True
            rc.font.color.rgb = RGBColor(0x25,0x63,0xeb)
            doc.add_paragraph()
        elif btype == 'hr':
            p = doc.add_paragraph()
...
TOOLS = [
    {
        ...
        "description": (
            "Gera PDF profissional com design livre e criativo via reportlab.\n"
            "FILOSOFIA: Cada PDF deve ser único e visualmente impressionante. Não use templates genéricos.\n"
            "OBRIGATÓRIO ao gerar:\n"
            "- Layout criativo: hero com fundo escuro, seções coloridas, tabelas elegantes, blocos de destaque\n"
            "- Paleta de cores coesa: escolha 2-3 cores principais e use consistentemente\n"
            "- Tipografia variada: tamanhos diferentes para hierarquia visual clara\n"
            "- Elementos visuais: barras coloridas, boxes destacados, ícones com texto\n"
            "- Adaptativo: 1 página para resumos, múltiplas para guias completos\n"
            "PARÂMETROS do usuário — obedeça SEMPRE:\n"
            "  'letras maiores' → fontSize base 11-13pt\n"
            "  'letras menores' → fontSize base 8-9pt\n"
            "  'perto do cabeçalho/topo' → hero_altura='pequeno' (topMargin=4cm)\n"
            "  '1 folha/página' → comprima todo o conteúdo, reduza fontes e espaços\n"
            "  'sem círculos' → sem_circulos=true\n"
            "  'compacto' → espacamento='compacto', fontes menores\n"
            "  tema de cores → aplique a paleta solicitada\n"
            "QUALIDADE:\n"
            "- Somente português brasileiro correto\n"
            "- NUNCA invente leis ou referências\n"
            "- Alíquota interna RO = 19,5% (Leis 5.629/2023 e 5.634/2023)\n"
            "- Revise o conteúdo antes de gerar"
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "titulo":         {"type": "string"},
                "conteudo":       {"type": "string", "description": "Conteúdo completo em markdown"},
...
2. PYTHON AVANÇADO: use executar_python para:
   ...
   - CRÍTICO ao gerar documentos: TODO o conteúdo DEVE estar em português brasileiro correto.
   - NUNCA use palavras em inglês, árabe, espanhol ou qualquer outro idioma. Nem uma palavra.
   - NUNCA invente referências normativas. Cite apenas leis que realmente existem.
   - NUNCA use backtick simples isolado — use ``` para blocos de código ou fórmula.
   - Alíquota interna de Rondônia (RO): 19,5% (desde jan/2024, Leis 5.629/2023 e 5.634/2023).
   - Quando o usuário pedir para MODIFICAR um documento, use os parâmetros corretos da tool gerar_pdf_documento
   - Interprete os pedidos livremente: 'mais bonito', 'melhor', 'diferente' → escolha tema, cores e layout que faça sentido
   - 'letras maiores' → tamanho_fonte='grande' | 'perto do topo' → hero_altura='pequeno'
   - '1 folha' → uma_pagina=true | 'sem círculos' → sem_circulos=true
   - 'mais compacto' → uma_pagina=true, espacamento='compacto', tamanho_fonte='pequeno'
   - Seja criativo com o layout — cada documento deve parecer profissional e único
...
def chat_tools():
        ...
        try:
            if tool_name == "gerar_grafico_barras":
                ...
                block = {
                    ...
                    "doc_markdown": tool_input["conteudo"][:5000]
                }
    ...
    if last_doc and isinstance(last_doc, dict) and last_doc.get("conteudo"):
        ...
        _cd = last_doc.get("conteudo","")[:3000]
        system_com_memoria += (
            f"\n\n ÚLTIMO DOCUMENTO GERADO \n"
            f"Título: {_td}\n"
            f"Conteúdo markdown:\n{_cd}\n"
            f"\nSe pedirem para modificar (letras maiores, compactar, mudar cores, etc), "
            f"use este conteúdo como base e aplique os parâmetros corretos da tool."
        )
