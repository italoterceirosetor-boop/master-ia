...
1. LER ARQUIVOS: PDF, Excel, Word, imagem, CSV — use ler_arquivo sempre que o usuário enviar arquivo.
...
def chat_tools():
    ...
    def _limpar_msg(m):
        ...
        if isinstance(cont, str):
            ...
            # Remove base64 inline de anexos antigos
            if "[ARQUIVO ANEXADO:" in txt and "Conteúdo base64:" in txt:
                nome = txt.split("]")[0].replace("[ARQUIVO ANEXADO:","").strip()
                txt = f"[Arquivo anexado: {nome}]"
            ...
            for b in cont:
                ...
                if btype == "text":
                    ...
                    # Remove base64 de anexos antigos no histórico
                    if txt.startswith("[ARQUIVO ANEXADO:") and "Conteúdo base64:" in txt:
                        nome = txt.split("]")[0].replace("[ARQUIVO ANEXADO:","").strip()
                        clean.append({"type":"text","text":f"[Arquivo anexado: {nome}]"})
                    elif txt:
    ...
    # Extrai texto do base64 AQUI no servidor, evitando que a API
    # precise chamar ler_arquivo (economiza 1 round-trip pelo proxy)
    if msgs_loop:
        last = msgs_loop[-1]
                        ...
                        if txt.startswith("[ARQUIVO ANEXADO:") and "Conteúdo base64:" in txt:
                            try:
                                ...
                                if b64:
                                    ...
                                    if len(texto) > 6000:
                                        texto = texto[:6000] + "\n\n[... truncado]"
                                    new_parts.append({"type": "text", "text": f"[Arquivo '{nome}']\n{texto}"})
                                    changed = True
                                    continue
                            except:
                                new_parts.append({"type": "text", "text": f"[Arquivo — erro ao ler]"})
                                changed = True
                                continue
    ...
    for iteration in range(MAX_ITER):
        # Tenta até 2x por iteração (retry em caso de 524 timeout do proxy)
        resp_data = None
        for tentativa in range(2):
            try:
                ...
                # Detecta resposta HTML (erro 524 Cloudflare)
                ct = resp.headers.get("content-type", "")
                if "text/html" in ct or resp.text.strip().startswith("<!DOCTYPE"):
                    if tentativa == 0:
                        import time; time.sleep(3)
                        continue  # retry
                    return jsonify({"erro": "Timeout do proxy (524). Tente novamente em alguns segundos."}), 504
                if not resp.ok:
                ...
                if tentativa == 0:
                    ...
                    continue
                return jsonify({"erro": "Timeout na requisição. Tente novamente."}), 504
            except Exception as e:
                return jsonify({"erro": str(e), "detalhe": tb.format_exc()}), 500

        if not resp_data:
            return jsonify({"erro": "Falha após retry"}), 504
