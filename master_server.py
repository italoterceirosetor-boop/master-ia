...
def gen_pdf(titulo, content, imagens=None, tema="padrao", uma_pagina=False, sem_circulos=False, subtitulo="", tamanho_fonte="normal", espacamento="normal", hero_altura="normal", mostrar_rodape=True):
    ...
    for block in blocks_all:
        ...
        if btype == 'h1':
            ...
            ncols = len(headers)
            # Largura máxima disponível (respeita margens)
            PW = A4[0] - 2*cm  # largura da página menos margens
            ...
            total = sum(lens) or 1
            cws = [max(PW * l / total, PW * 0.06) for l in lens]
            sw  = sum(cws); cws = [w * PW / sw for w in cws]
            # Reduz fonte automaticamente para tabelas largas
            fs  = 6.0 if ncols > 7 else 7.0 if ncols > 5 else 8.5
...
def chat_tools():
    ...
    def _gerar_sse():
        ...
        MAX_ITER = 8
        stop_reason = ""
        _msgs = list(msgs_loop)

        for iteration in range(MAX_ITER):
            # Tenta APIs em ordem, com fallback
            resp = None
            last_exc = None
            for _url in _API_URLS:
                try:
                    ...
                    if resp.ok:
                        break
                    elif resp.status_code in (502, 503, 504, 524, 1033):
                        last_exc = Exception(f"API indisponível ({resp.status_code})")
                        resp = None
                        continue
                    else:
                        yield f"data: {_json_sse.dumps({'erro': resp.text})}\n\n"
                        return
                except Exception as e:
                    last_exc = e
                    resp = None
            ...
            if resp is None:
                yield f"data: {_json_sse.dumps({'erro': str(last_exc)})}\n\n"
                return

            try:
                data = resp.json()
            except Exception as e:
                yield f"data: {_json_sse.dumps({'erro': str(e)})}\n\n"
                return
            ...
            for block in content:
                ...
                if btype == "text":
                    ...
                    if txt:
                        bloco = {"tipo": "texto", "conteudo": txt}
                        result_blocks.append(bloco)
                        # Emite texto imediatamente via SSE
                        yield f"data: {_json_sse.dumps({'bloco': bloco})}\n\n"
                elif btype == "tool_use":
            ...
            # Executa ferramentas e emite progresso
            tool_results = []
            for tu in tool_uses:
                ...
                # Avisa que está executando a ferramenta
                _label_map = {
                    "gerar_pdf": "📄 Gerando PDF...",
                    "gerar_word": "📝 Gerando Word...",
                    "gerar_excel_completo": "📊 Gerando Excel...",
                    "executar_codigo_python": "🐍 Executando código...",
                    "ler_arquivo": "📂 Lendo arquivo...",
                    "buscar_web": "🔍 Buscando na web...",
                    "gerar_grafico_barras": "📊 Gerando gráfico...",
                    "gerar_grafico_linhas": "📈 Gerando gráfico...",
                    "gerar_grafico_pizza": "🥧 Gerando gráfico...",
                    "gerar_grafico_dispersao": "📉 Gerando gráfico...",
                }
                label = _label_map.get(tool_name, f"⚙ Executando {tool_name}...")
                yield f"data: {_json_sse.dumps({'status': label})}\n\n"

                rb, result_text = executar_ferramenta(tool_name, tool_input)
                result_blocks.append(rb)

                # Emite o bloco do resultado da ferramenta imediatamente
                yield f"data: {_json_sse.dumps({'bloco': rb})}\n\n"
            ...
            _msgs.append({"role": "assistant", "content": content})
            _msgs.append({"role": "user", "content": tool_results})

        registrar_evento("mensagem", user)
        # Evento final com todos os blocos (compatibilidade com clientes antigos)
        yield f"data: {_json_sse.dumps({'done': True, 'blocos': result_blocks, 'stop_reason': stop_reason})}\n\n"
