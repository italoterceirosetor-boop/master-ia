...
class _MasterPDFDoc(BaseDocTemplate):
    def __init__(self, buf, cores, sem_circulos=False, **kw):
        self._cores = cores
    ...
    def _bg(self, c, doc):
        ...
        W2, H2 = A4
        c.saveState()
        c.setFillColor(colors.HexColor(C['bg']));   c.rect(0, H2-6*cm, W2, 6*cm, fill=1, stroke=0)
        c.setFillColor(colors.HexColor(C['cyan'])); c.rect(0, H2-0.28*cm, W2, 0.28*cm, fill=1, stroke=0)
        c.setFillColor(colors.HexColor(C['gold'])); c.rect(0, H2-6*cm-0.14*cm, W2, 0.14*cm, fill=1, stroke=0)
        if not self._sem_circulos:
            c.setFillColor(colors.HexColor(C['navy'])); c.circle(W2-0.7*cm, H2-3*cm, 2.4*cm, fill=1, stroke=0)
            c.setFillColor(colors.HexColor(C['blue'])); c.circle(W2-0.7*cm, H2-3*cm, 1.5*cm, fill=1, stroke=0)
            c.setFillColor(colors.HexColor(C['cyan'])); c.circle(W2-0.7*cm, H2-3*cm, 0.55*cm, fill=1, stroke=0)
        c.setFillColor(colors.HexColor(C['cyan'])); c.rect(0, 0, 0.16*cm, H2-6.14*cm, fill=1, stroke=0)
...
def gen_pdf(titulo, content, imagens=None, tema="padrao", uma_pagina=False, sem_circulos=False, subtitulo=""):
    ...
    # Escala para 1 página
    sc = 0.82 if uma_pagina else 1.0
    # Estilos locais
    S = {
    ...
    buf = io.BytesIO()
    doc = _MasterPDFDoc(buf, cores=t, sem_circulos=sem_circulos, pagesize=A4,
        leftMargin=1.8*cm, rightMargin=1.8*cm,
        topMargin=6.8*cm, bottomMargin=1.8*cm,
...
TOOLS = [
    {
        ...
        "description": (
            "Gera um documento PDF profissional formatado. Use quando o usuário pedir um PDF, guia, "
            "relatório, resumo, documento educativo, manual ou qualquer conteúdo para baixar em PDF. "
...
def chat_tools():
    ...
    if last_doc and isinstance(last_doc, dict) and last_doc.get("conteudo"):
        ...
        system_com_memoria += (
            ...
            f"Título: {_td}\n"
            f"Markdown:\n{_cd}\n"
            f"\nSe pedirem para modificar este documento, use como base e aplique as mudanças."
        )
