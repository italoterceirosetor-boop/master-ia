...
app = Flask(__name__)
CORS(app, resources={r"/*": {"origins": "*", "allow_headers": ["X-Token", "Content-Type", "Authorization"]}})
...
def _limpar_texto_pdf(txt: str) -> str:
    ...
    return "\n".join(l for l in lines if sum(1 for c in l if c.isalnum()) >= 2 or not l.strip()).strip()
...
def chat_tools():
    ...
    def _extrair_arquivo_de_texto(txt):
        ...
        def _sub(mo):
            ...
            try:
                return "[Arquivo: " + nome + "]\n\n" + tool_ler_arquivo(b64, nome)
            except Exception as e:
                return "[Arquivo: " + nome + " — erro: " + str(e) + "]"
    ...
    def _preprocessar(m):
        ...
        if isinstance(cont, list):
            return {"role": role, "content": [
                {"type":"text","text":_extrair_arquivo_de_texto(b.get("text",""))} if b.get("type")=="text" else b
                for b in cont
            ]}
