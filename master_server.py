...
def gen_pdf(titulo, content, imagens=None, tema="padrao", uma_pagina=False, sem_circulos=False, subtitulo=""):
    ...
    for block in blocks_all:
        ...
        if btype == 'h1':
            t = Table([[Paragraph(txt, _S['h1'])]], colWidths=[CW])
            t.setStyle(TableStyle([
                ('BACKGROUND',(0,0),(-1,-1),colors.HexColor('#eff6ff')),
                ('LINEBEFORE',(0,0),(0,-1),4,_C['blue']),
            ...
            story.append(Paragraph(txt, _S['h2']))
            story.append(HRFlowable(width='100%', thickness=0.7, color=_C['surf2'], spaceAfter=3))

        elif btype == 'h3':
            story.append(Paragraph(txt, _S['h3']))

        elif btype in ('h4',):
            story.append(Paragraph(txt, _ps('mh4tmp', fontSize=9, fontName='Helvetica-BoldOblique',
                         textColor=_C['text2'], leading=13)))

        elif btype == 'para':
            story.append(Paragraph(txt, _S['body']))

        elif btype == 'bullets':
            for item in block['items']:
                story.append(Paragraph(f'<font color="#2563eb" size="11">•</font>  {item}', _S['bullet']))

        elif btype == 'numbered':
            for n, item in enumerate(block['items'], 1):
                story.append(Paragraph(f'<b><font color="#2563eb">{n}.</font></b>  {item}', _S['num']))

        elif btype == 'cite':
            t = Table([[Paragraph(txt, _S['cite'])]], colWidths=[CW])
            t.setStyle(TableStyle([
                ('BACKGROUND',(0,0),(-1,-1),_C['surf']),
                ('LINEBEFORE',(0,0),(0,-1),3,_C['cyan']),
            ...
            s_th2 = _ps(f'TH{uid}', fontSize=fs, fontName='Helvetica-Bold', textColor=_C['white'],  alignment=TA_CENTER, leading=fs+2)
            s_td2 = _ps(f'TD{uid}', fontSize=fs, fontName='Helvetica',      textColor=_C['text'],   alignment=TA_LEFT,   leading=fs+4)
            s_tn2 = _ps(f'TN{uid}', fontSize=fs, fontName='Helvetica',      textColor=_C['text'],   alignment=TA_RIGHT,  leading=fs+4)
            def _is_num(v):
            ...
            t.setStyle(TableStyle([
                ...
                ('LINEBELOW',(0,0),(-1,0),2,_C['cyan']),
                ('ROWBACKGROUNDS',(0,1),(-1,-1),[_C['stripe'],_C['white']]),
                ('GRID',(0,0),(-1,-1),0.4,_C['surf2']),
    ...
    story.append(HRFlowable(width='100%', thickness=0.4, color=_C['surf2'], spaceAfter=3))
    story.append(Paragraph(
        'Gerado por Master IA · Este documento tem caráter informativo.', _S['foot']))
