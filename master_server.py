...
def _md2rl(text):
    ...
    text = _re.sub(r'`(.+?)`',         r'<font name="Courier" size="8">\1</font>', text)
...
def gen_pdf(titulo, content, imagens=None, tema="padrao", uma_pagina=False, sem_circulos=False, subtitulo="", tamanho_fonte="normal", espacamento="normal", hero_altura="normal", mostrar_rodape=True):
    ...
    # Escala para 1 página (sobrescreve fonte se muito_grande)
    ...
    sp_after_hero = (_hero_band + 0.0*cm) if not uma_pagina else (_hero_band - 0.4*cm)
...
2. PYTHON AVANÇADO: use executar_python para:
   ...
   - CRÍTICO ao gerar documentos: TODO o conteúdo DEVE estar em português brasileiro correto. NUNCA use palavras em inglês. Revise o código antes de executar.
