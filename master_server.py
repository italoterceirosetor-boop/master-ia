...
class _MasterPDFDoc(BaseDocTemplate):
    ...
    def _bg(self, c, doc):
        ...
        if not self._sem_circulos:
            cy = H2 - hb/2
            c.setFillColor(colors.HexColor(C['navy'])); c.circle(W2-0.7*cm, cy, 2.4*cm, fill=1, stroke=0)
            c.setFillColor(colors.HexColor(C['blue'])); c.circle(W2-0.7*cm, cy, 1.5*cm, fill=1, stroke=0)
            c.setFillColor(colors.HexColor(C['cyan'])); c.circle(W2-0.7*cm, cy, 0.55*cm, fill=1, stroke=0)
...
def _md2rl(text):
    ...
    text = text.replace('`', '')  # Remove backticks soltos
...
def gen_pdf(titulo, content, imagens=None, tema="padrao", uma_pagina=False, sem_circulos=False, subtitulo="", tamanho_fonte="normal", espacamento="normal", hero_altura="normal", mostrar_rodape=True):
    ...
    _top_margin   = _hero_band + 1.2*cm
    sp_hero       = (_hero_band - 1.0*cm) if not uma_pagina else (_hero_band - 1.3*cm)
    sp_after_hero = _hero_band if not uma_pagina else (_hero_band - 0.4*cm)
    ...
    for block in blocks_all:
        ...
        if btype == 'h1':
            ...
            for r in rows:
                tdata.append([Paragraph(str(r[j]) if j<len(r) else '',
...
TOOLS = [
    {
        ...
        "description": (
            "Gera PDF profissional. Use para qualquer pedido de PDF, guia, relatório, resumo ou documento.\n"
            ...
            "- Alíquota interna RO = 19,5% (desde jan/2024, Leis 5.629/2023 e 5.634/2023).\n"
            "PARÂMETROS DE FORMATAÇÃO — use SEMPRE que o usuário pedir mudança visual:\n"
            "  tamanho_fonte: 'pequeno'(8pt) | 'normal'(9pt) | 'grande'(11pt) | 'muito_grande'(13pt)\n"
            "  hero_altura: 'pequeno'(3cm) | 'normal'(6cm) | 'grande'(8cm)\n"
            "  espacamento: 'compacto' | 'normal' | 'espacoso'\n"
            "  tema: 'padrao' | 'verde' | 'roxo' | 'escuro'\n"
            "  uma_pagina: true | false\n"
            "  sem_circulos: true | false\n"
            "  mostrar_rodape: true | false\n"
            "MAPEAMENTO pedido → parâmetro:\n"
            "  'letras maiores' → tamanho_fonte='grande'\n"
            "  'mais perto do cabeçalho' → hero_altura='pequeno'\n"
            "  'letras menores' → tamanho_fonte='pequeno'\n"
            "  'só uma página / mais compacto' → uma_pagina=true, espacamento='compacto'\n"
            "  'sem círculos' → sem_circulos=true\n"
            "  'cores verdes/roxas/escuras' → tema correspondente"
        ),
        "input_schema": {
            "type": "object",
            "properties": {
...
def tool_buscar_web(query: str, num_resultados: int = 5) -> str:
    ...
    try:
        ...
        if not resultados:
            return f"Busca realizada para '{query}' — nenhum resultado encontrado."
...
2. PYTHON AVANÇADO: use executar_python para:
   ...
   - NUNCA use palavras em inglês, árabe, indonésio ou qualquer outro idioma. Nem uma palavra.
   - NUNCA invente referências normativas. Cite apenas leis que realmente existem.
   - NUNCA use backtick simples isolado no conteúdo — use ``` para blocos de código/fórmula.
   - Alíquota interna de Rondônia (RO): 19,5% (vigente desde janeiro de 2024, Leis 5.629/2023 e 5.634/2023).
...
def chat_stream():
    """Streaming SSE para respostas de texto simples."""
    user = auth(request)
    if not user: return jsonify({"erro":"Não autenticado"}), 401
    d        = request.get_json() or {}
