...
1. LER ARQUIVOS: PDF, Excel, Word, CSV — use ler_arquivo sempre que o usuário enviar arquivo.
   ...
   Descreva o que vê, extraia dados, analise documentos, notas fiscais, prints de sistemas, etc.
   NÃO use ler_arquivo para imagens — você já as vê nativamente.
    ...
    if msgs_loop:
        ...
        if last.get("role") == "user":
            ...
            if isinstance(content, list):
                new_parts = []
                changed = False
                # Extensões de imagem que a API Anthropic aceita nativamente (vision)
                IMAGENS_VISION = {
                    ...
                    if part.get("type") == "text":
                        ...
                        if txt.startswith("[ARQUIVO ANEXADO:") and "Conteúdo base64:" in txt:
                            try:
                                ...
                                if b64:
                                    # ── VISION NATIVA: imagens vão como image block ──────────
                                    if ext in IMAGENS_VISION:
                                        media_type = IMAGENS_VISION[ext]
                                        # Legenda antes da imagem para o Claude ter contexto
                                        new_parts.append({
                                            "type": "text",
                                            "text": f"[Imagem enviada pelo usuário: '{nome}']"
                                        })
                                        new_parts.append({
                                            "type": "image",
                                            "source": {
                            ...
                            # Extração falhou ou vazia — mantém original pra API processar
