...
def _limpar_texto_pdf(txt: str) -> str:
    """Limpa artefatos comuns de extração de PDF."""
    ...
    # Substitui blocos de símbolos (■□▪▫●◦) que surgem de fontes corrompidas
    txt = _re2.sub(r"[\u25a0-\u25ff\u2600-\u26ff\uf000-\uffff]+", "", txt)
    # Remove sequências de caracteres privados Unicode (PUA)
    txt = _re2.sub(r"[\ue000-\uf8ff]+", "", txt)
    # Normaliza espaços múltiplos mas preserva quebras de linha
    lines = [_re2.sub(r" {2,}", " ", line) for line in txt.split("\n")]
    txt = "\n".join(lines)
    # Remove linhas que só têm símbolos/ruído (< 3 caracteres alfanuméricos)
    clean_lines = []
    for line in txt.split("\n"):
        alnum = sum(1 for c in line if c.isalnum())
        if alnum >= 2 or line.strip() == "":
