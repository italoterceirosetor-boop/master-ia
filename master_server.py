...
def chat_tools():
        ...
        if isinstance(cont, str):
            ...
            for b in cont:
                ...
                if btype == "text":
                    ...
                    # Imagens: mantém no histórico (a API aceita)
                    # Nota: imagens antigas são mantidas pois a API Anthropic precisa delas para contexto
                    clean.append(b)
                elif btype == "document":
                    clean.append(b)
    ...
    # ── Pré-processa anexos de arquivo na ÚLTIMA mensagem do usuário ──
    # O frontend envia: { type:'text', text:'[ARQUIVO ANEXADO: nome]\nConteúdo base64: XXX...' }
    # Aqui extraímos o base64, lemos o arquivo, e substituímos pelo texto extraído.
    # Isso evita que o base64 gigante seja reenviado a cada turno do histórico.
    def _processar_anexos_inline(msgs):
        if not msgs:
            return msgs
        last = msgs[-1]
            ...
            if part.get("type") == "text":
                ...
                if txt.startswith("[ARQUIVO ANEXADO:") and "Conteúdo base64:" in txt:
                    try:
                        ...
                        if b64:
                            texto_extraido = tool_ler_arquivo(b64, nome, False)
                            if len(texto_extraido) > 12000:
                                texto_extraido = texto_extraido[:12000] + "\n\n[... conteúdo truncado — arquivo muito grande]"
                            new_parts.append({
                                "type": "text",
                                "text": f"[Conteúdo do arquivo '{nome}']\n{texto_extraido}"
                            })
                            changed = True
                            continue
                    except Exception as ex:
                        nome_err = txt.split("]")[0].replace("[ARQUIVO ANEXADO:", "").strip()
                        new_parts.append({"type": "text", "text": f"[Arquivo '{nome_err}' — erro ao ler: {ex}]"})
                        changed = True
                        continue
            new_parts.append(part)
        if changed:
            msgs[-1] = {"role": "user", "content": new_parts}
        return msgs

    msgs_loop = _processar_anexos_inline(msgs_loop)
