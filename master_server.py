...
def _limpar_texto_pdf(txt: str) -> str:
    """Remove artefatos de extração: símbolos ■, caracteres privados, lixo de fonte."""
    import unicodedata, re as _r
    txt = "".join(c for c in txt if unicodedata.category(c)[0] != "C" or c in "\n\t\r")
    txt = _r.sub(r"[\u25a0-\u25ff\u2600-\u26ff\uf000-\uffff]+", "", txt)
    txt = _r.sub(r"[\ue000-\uf8ff]+", "", txt)
    lines = [_r.sub(r" {2,}", " ", l) for l in txt.split("\n")]
    clean = [l for l in lines if sum(1 for c in l if c.isalnum()) >= 2 or l.strip() == ""]
    return "\n".join(clean).strip()
    ...
    try:
        ...
        with pdfplumber.open(io.BytesIO(pdf_data)) as pdf:
            ...
            for i, page in enumerate(pdf.pages, 1):
                txt = _limpar_texto_pdf(page.extract_text(x_tolerance=3, y_tolerance=3) or "")
                if txt.strip():
                    textos.append(f"--- Página {i} ---\n{txt.strip()}")
        ...
        try:
            ...
            for i, page in enumerate(reader.pages, 1):
                txt = _limpar_texto_pdf(page.extract_text() or "")
                if txt.strip():
                    textos.append(f"--- Página {i} ---\n{txt.strip()}")
            return "\n\n".join(textos)
        except:
...
2. PYTHON AVANÇADO — AÇÃO COMO PROGRAMADORA SÊNIOR:
   ...
   - Responda no idioma do usuário. Se ele escrever em espanhol, responda em espanhol.
...
def chat_tools():
    ...
    # ── Extrai texto de arquivos ANTES de mandar para API (evita timeout 524) ──
    def _extrair_arquivo_de_texto(txt):
        import re as _r
        pat = _r.compile(
            r'\[ARQUIVO ANEXADO: ([^\]]+)\](?:\\n|\n)Conte[uú]do base64: ([A-Za-z0-9+/=\n\\]+)(?:\\n|\n)Use a ferramenta[^\n]*',
            _r.DOTALL
        )
        def _sub(mo):
            nome = mo.group(1).strip()
            b64  = mo.group(2).strip().replace("\\n","").replace("\n","")
            try:
                conteudo = tool_ler_arquivo(b64, nome)
                return "[Arquivo: " + nome + "]\n\n" + conteudo
            except Exception as e:
                return "[Arquivo: " + nome + " — erro ao ler: " + str(e) + "]"
    ...
    def _preprocessar(m):
        ...
        if isinstance(cont, list):
            ...
            for b in cont:
                if b.get("type") == "text":
                    partes.append({"type":"text","text":_extrair_arquivo_de_texto(b.get("text",""))})
                else:
                    partes.append(b)
            return {"role": role, "content": partes}
        elif isinstance(cont, str):
            return {"role": role, "content": _extrair_arquivo_de_texto(cont)}
        return m

    messages = [_preprocessar(m) for m in messages]
    ...
    for iteration in range(MAX_ITER):
        ...
        for _url in _API_URLS:
            try:
                ...
                continue
        if resp is None:
            return jsonify({"erro": str(last_exc)}), 500
        try:
            data = resp.json()
        except Exception as e:
