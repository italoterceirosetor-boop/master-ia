...
@app.route("/ler_arquivo_rapido", methods=["POST"])
def ler_arquivo_rapido():
    """Lê arquivo e retorna texto extraído — sem chamar API."""
    user = auth(request)
    if not user: return jsonify({"erro":"Não autenticado"}), 401
    d = request.get_json() or {}
    b64  = d.get("b64","")
    nome = d.get("nome","arquivo")
    if not b64:
        return jsonify({"erro":"b64 obrigatório"}), 400
    try:
        texto = tool_ler_arquivo(b64, nome)
        return jsonify({"texto": texto, "nome": nome})
